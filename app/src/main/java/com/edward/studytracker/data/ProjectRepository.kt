    ...
    suspend fun importData(json: String): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            val importData = try {
                ...
                throw IllegalArgumentException("Unexpected JSON content", e)
            }
            ...
            val projects = requireNotNull(importData.projects) { "Data is missing the project list" }
            val studyUnits = requireNotNull(importData.studyUnits) { "Data is missing the unit list" }
            val problems = requireNotNull(importData.problems) { "Data is missing the problem list" }
            val practiceRecords = requireNotNull(importData.practiceRecords) { "Data is missing the practice records" }
